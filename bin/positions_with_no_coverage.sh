...
Aav=$(samtools depth -a -r "JQ995537:25633-26964" $1  | awk '{s+=$3}END{if (NR>0) {print s/NR} else {print "0"}}')
Bav=$(samtools depth -a -r "JQ995537:33708-35062" $1  | awk '{s+=$3}END{if (NR>0) {print s/NR} else {print "0"}}')
Cav=$(samtools depth -a -r "JQ995537:43819-45057" $1  | awk '{s+=$3}END{if (NR>0) {print s/NR} else {print "0"}}')
